<?php

return [
    'hi' => 'Hi,',
    'this_is_a_test_email' => 'Detta är ett testmail. Vänligen ignorera det.',
    'config_is_working' => 'Om du ser detta e-postmeddelande betyder det att e-postkonfigurationen fungerar.',
    'all_rights_reserved' => 'Alla rättigheter reserverade.',
    'subject_test_email' => 'Test Email',
    'user_details' => 'Användardetaljer',
    'name' => 'Namn',
    'email' => 'E-post',
    'password' => 'Lösenord',
    'click_on_the_button_below_to_login' => 'Klicka på knappen nedan för att logga in.',
    'login' => 'Inloggning',
    'subject_login_details' => 'Inloggningsuppgifter',
    'hi_name' => 'Hej :name,',
    'new_invoice_created_for_you_or_your_company' => 'En ny faktura har skapats för dig eller ditt företag.',
    'invoice_details' => 'Faktura Detaljer',
    'invoice_number' => 'Fakturanummer: :nummer',
    'invoice_date' => 'Faktura datum: :date',
    'due_date' => 'Sista inlämningsdatum: :date',
    'amount' => 'Totalt belopp: :amount',
    'click_on_the_button_below_to_view_invoice' => 'Klicka på knappen nedan för att se fakturan.',
    'view_invoice' => 'Visa faktura',
    'thank_you' => 'Tack för ditt företag!',
    'subject_new_invoice' => 'Ny fakturaavisering',
    'new_quote_created_for_you_or_your_company' => 'Ett nytt citat har skapats för dig eller ditt företag.',
    'quote_details' => 'Offert Detaljer',
    'quote_number' => 'Offertnummer: :number',
    'quote_date' => 'Offert datum: :date',
    'valid_until' => 'Giltig Until: :date',
    'click_on_the_button_below_to_view_quote' => 'Klicka på knappen nedan för att se offerten.',
    'view_quote' => 'Visa offert',
    'subject_new_quote' => 'Ny offertavisering',
    'new_contract_created_for_you' => 'Ett nytt kontrakt har skapats för dig, vänligen skriv under det.',
    'contract_details' => 'Kontrakt Detaljer',
    'contract_number' => 'Kontraktsnummer: :number',
    'contract_expiry_date' => 'Förfallodatum för signering: datum',
    'click_on_the_button_below_to_sign_contract' => 'Klicka på knappen nedan för att skriva under avtalet.',
    'sign_contract' => 'Signera kontrakt',
    'subject_new_contract' => 'Nytt kontraktsmeddelande',
    'login_notification' => 'Notifiering för inloggning',
    'login_details' => 'Inloggningsuppgifter',
    'subject_login_notification' => 'Notifiering för inloggning',
    'new_login_to_your_account' => 'En ny inloggning till ditt konto har upptäckts.',
    'login_time' => 'Inloggningstid: :time',
    'login_ip' => 'IP-adress: :ip',
    'login_location' => 'Plats: :lat :lon',
    'login_browser' => 'Webbläsare: :browser',
    'login_os' => 'OS: :os',
    'login_date' => 'Datum och tid: :date',
    'if_this_was_not_you' => 'Om detta inte var du, vänligen ändra ditt lösenord omedelbart och kontakta administratören.',
    'personal_access_token_created' => 'Personlig åtkomsttoken skapad',
    'subject_personal_access_token_created' => 'Personlig åtkomsttoken skapad',
    'new_personal_access_token_generated' => 'En ny personlig åtkomsttoken har skapats på ditt konto.',
    'personal_access_token' => 'Personlig åtkomsttoken',
    'personal_access_token_details' => 'Personliga åtkomsttoken detaljer',
    'personal_access_token_name' => 'Token Namn',
    'personal_access_token_valid_until' => 'Giltig till',
    'if_this_was_not_you_personal_access_token' => 'Om detta inte var du, ta bort token omedelbart och kontakta administratören.',
    'subject_support_ticket' => 'Support ärendeavisering',
    'new_support_ticket' => 'Ett nytt supportärende har skapats.',
    'support_ticket_details' => 'Support Ärendeuppgifter',
    'support_ticket_number' => 'Antal: :number',
    'support_ticket_status' => 'Status: :status',
    'support_ticket_subject' => 'Angående: Ämne',
    'click_on_the_button_below_to_view_ticket' => 'Klicka på knappen nedan för att se biljetten.',
    'view_ticket' => 'Visa ärende',
    'new_user_for_client_portal' => 'En ny användare har skapats för klientportalen.',
    'client_portal_details' => 'Åtkomstdetaljer för klientportalen',
    'client_portal_email' => 'Client Portal Email: :email',
    'client_portal_password' => 'Lösenord för kundportalen:',
    'login_to_client_portal' => 'Logga in till klientportalen',
    'client_portal_notification_subject' => 'Meddelande om åtkomst till klientportalen',
];
